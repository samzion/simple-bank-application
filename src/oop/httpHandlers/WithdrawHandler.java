...
public class WithdrawHandler extends BaseHandler implements HttpHandler {
    public void handle (HttpExchange exchange) throws IOException {
        ...
        try{
            ...
            String withdrawResponse =accountService.withdraw(authenticatedUser, withdrawRequest.getAccountNumber()
                    , withdrawRequest.getWithdrawAmount());
            if(withdrawResponse.equalsIgnoreCase("Withdrawal request successful!")){
                SimpleBankRestApiApplication.writeHttpResponse(exchange, 200, withdrawResponse);
                return;
            }
            if(withdrawResponse.equalsIgnoreCase("You have no existing account")){
                SimpleBankRestApiApplication.writeHttpResponse(exchange, 404, withdrawResponse);
                return;
            }
            if(withdrawResponse.equalsIgnoreCase("Invalid account number")){
                SimpleBankRestApiApplication.writeHttpResponse(exchange, 400, withdrawResponse);
            }
